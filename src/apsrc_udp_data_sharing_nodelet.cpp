...
void ApsrcUdpDataSharingNl::hereAppCallback(const apsrc_msgs::Response::ConstPtr& msg)
{
  ...
  for (int ridx = 0; ridx < number_of_routes; ridx++){
    ...
    udp_msg.summary_here.number_of_waypoint = static_cast<float>(msg->routes[ridx].waypoints.size()/3);
    if (udp_msg.summary_here.number_of_waypoint > 2665){
      ROS_WARN("Number of Waypoints exceeded max capacity! Extra points dropped ...");
      udp_msg.summary_here.number_of_waypoint = static_cast<float>(2665);
    }
    udp_msg.summary_here.length = static_cast<float>(msg->routes[ridx].distance/1);
    udp_msg.summary_here.duration = static_cast<float>(msg->routes[ridx].duration/1);
    udp_msg.summary_here.base_duration = static_cast<float>(msg->routes[ridx].baseDuration/1);
